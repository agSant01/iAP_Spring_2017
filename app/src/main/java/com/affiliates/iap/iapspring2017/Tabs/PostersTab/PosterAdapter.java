...
package com.affiliates.iap.iapspring2017.Tabs.PostersTab;
...
public class PosterAdapter extends ArrayAdapter<Poster> {
    ...
    public View getView(int position, View convertView, ViewGroup viewGroup){
        ...
        if(ConstantsService.getCurrentLoggedInUser().getAccountType() == User.AccountType.CompanyUser){
            if( ((CompanyUser) ConstantsService.getCurrentLoggedInUser()).hasEvaluated(poster.getPosterID())){
            ...
            if(ConstantsService.getCurrentLoggedInUser().hasVoted(0) && ConstantsService.getCurrentLoggedInUser().hasVoted(1)){
                viewHolder.mPosterNum.setVisibility(View.INVISIBLE);
                viewHolder.mHasEval.setVisibility(View.INVISIBLE);
                viewHolder.check.setVisibility(View.VISIBLE);
            }
